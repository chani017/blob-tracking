...
### 데이터 시각화 & 인터랙션
- 다국어 지원 (Bilingual Support): 상단 토글을 통해 툴팁 및 가이드를 한국어와 영어 중 선택하여 볼 수 있습니다.
- 동적 라벨링: 객체의 순서(ID) 또는 화면 대비 상대적 크기(Size) 데이터를 실시간 텍스트로 표시합니다.
- 인터랙티브 가이드: 각 설정 항목 옆의 i 아이콘을 통해 기능별 상세 설명을 확인할 수 있습니다.

### 기술적 최적화 (Technical Excellence)
- Zero-Allocation Loop: 실시간 트래킹 루프 내에서 가비지 컬렉션(GC)을 유발하는 메모리 할당을 제거하여 극강의 프레임 안정성을 보장합니다.
- 효율적인 리소스 관리: 비디오 업로드 및 녹화 시 사용되는 ObjectURL의 명시적 해제를 통해 장시간 사용 시의 메모리 누수를 원천 차단했습니다.
- 비디오 처리 & 녹화: 로컬 비디오 업로드를 지원하며, 모든 효과가 포함된 결과물을 고품질 60FPS MP4 동영상으로 저장 가능합니다.
